...
def wireless_channel_transition_probability(clients):
    if clients_state == []:
        ...
        for i in range(clients):
            clients_state.append(random.randint(0, 1))
    else:
        # print('This is Not 0')
        for i in range((clients)):
            clients_state[i] = random.randint(0, 1)
...
if __name__ == '__main__':
    ...
    if os.path.exists(f"./mnist_results/fedavg_acc_lam_{lam}_k_{top_k}_random.txt"):

        os.remove(f"./mnist_results/fedavg_acc_lam_{lam}_k_{top_k}_random.txt")
        os.remove(
            f"./mnist_results/fedavg_loss_lam_{lam}_k_{top_k}_random.txt")
        os.remove(
            f"./mnist_results/fedavg_power_lam_{lam}_k_{top_k}_random.txt")
        os.remove(f"./mnist_results/ibcs_acc_lam_{lam}_k_{top_k}_random.txt")
        os.remove(f"./mnist_results/ibcs_loss_lam_{lam}_k_{top_k}_random.txt")
        os.remove(f"./mnist_results/ibcs_power_lam_{lam}_k_{top_k}_random.txt")

    acc_file_fedavg = open(
        f"./mnist_results/fedavg_acc_lam_{lam}_k_{top_k}_random.txt", "a")
    loss_file_fedavg = open(
        f"./mnist_results/fedavg_loss_lam_{lam}_k_{top_k}_random.txt", "a")
    power_file_fedavg = open(
        f"./mnist_results/fedavg_power_lam_{lam}_k_{top_k}_random.txt", "a")

    acc_file_ibcs = open(
        f"./mnist_results/ibcs_acc_lam_{lam}_k_{top_k}_random.txt", "a")
    loss_file_ibcs = open(
        f"./mnist_results/ibcs_loss_lam_{lam}_k_{top_k}_random.txt", "a")
    power_file_ibcs = open(
        f"./mnist_results/ibcs_power_lam_{lam}_k_{top_k}_random.txt", "a")
    ...
    clients_state.clear()
    print('client_state cleared only once before the training starts')
...
#######################################################################################
        ...
        # update global weights
        if w_locals == []:
            print('ibcs, no successfull users')
            continue
